...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        fr = open('./lanes_per_junction_2x2', 'rb')
        self.lanes_dict = pickle.load(fr)
    ...
    def reset(self, seed: Optional[int] = None, **kwargs):
        ...
        '''
        junctions={}
        for t in self.traffic_signals:
            junctions[t]=list(dict.fromkeys(self.sumo.trafficlight.getControlledLanes(t)))
        print(junctions)
        self.close()'''
    ...
    def _compute_rewards(self):
        self.rewards.update({ts: self.traffic_signals[ts].compute_reward(0) for ts in self.ts_ids if self.traffic_signals[ts].time_to_act})
        return {ts: self.rewards[ts] for ts in self.rewards.keys() if self.traffic_signals[ts].time_to_act}


    # passando 0 in compute_reward() uso la prima reward function, con 1 uso la seconda
    '''
    def _compute_rewards(self):
        if self.sumo.simulation.getTime()< self.sim_max_time/2:
            self.rewards.update({ts: self.traffic_signals[ts].compute_reward(0) for ts in self.ts_ids if self.traffic_signals[ts].time_to_act})
            return {ts: self.rewards[ts] for ts in self.rewards.keys() if self.traffic_signals[ts].time_to_act}
        else:
            if self.sumo.simulation.getTime()>= self.sim_max_time/2:
    ...
    def _sumo_step(self):
        #traci.gui.screenshot("View #0", "./i.png")
    ...
    def encode(self, state, ts_id):
        #phase = int(np.where(state[:len(self.traffic_signals[ts_id].actions)] == 1)[0])
        ...
        #density_queue = [self._discretize_density(d) for d in state[4+ 1:]]

        # tuples are hashable and can be used as key in python dictionary
        #return tuple([phase, min_green] + density_queue)
        return tuple(density_queue)
        '''
    def encode(self, state, ts_id):
...
class SumoEnvironmentPZ(AECEnv, EzPickle):
    ...
    def step(self, action):
        ...
        for i in traci.vehicle.getIDList():
            traci.vehicle.setSpeedMode(i, 55)
        '''
